...
def _collect_project_key_result_ids(project: Project | None) -> list[str]:
    key_result_ids: list[str] = []
    if not project:
        return key_result_ids

    for objective in project.objectives or []:
        for key_result in objective.key_results or []:
            if key_result.id:
...
async def delete_meeting(
    meeting_id: str,
    delete_request: Optional[MeetingDeleteRequest] = None,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
    ...
    delete_related = bool(delete_request.delete_related) if delete_request else False
    ...
    if delete_related:
        ...
        if meeting.project_id:
            project_result = await db.execute(
                ...
                .options(selectinload(Project.objectives).selectinload(Objective.key_results))
            )
            project = project_result.scalar_one_or_none()

            if project:
                other_meetings_result = await db.execute(
                    select(func.count(Meeting.id)).where(
                        Meeting.project_id == project.id,
                        Meeting.id != meeting_id,
                    )
                )
                other_meetings_count = other_meetings_result.scalar() or 0

                if other_meetings_count == 0:
                    related_key_result_ids = _collect_project_key_result_ids(project)
                    if related_key_result_ids:
                        await db.execute(
                            delete(Task).where(
                                Task.created_by == current_user.id,
                                Task.key_result_id.in_(related_key_result_ids),
                            )
                        )
